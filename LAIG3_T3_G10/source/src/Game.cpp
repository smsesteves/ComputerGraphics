...
void Game::rotateCamera(YAFScene* scene, int turn, int lastturn){
	//cout << "Rotating Camera " << endl;
...
void Game::unhighlightAll(){
	...
	for(unsigned int i = 0; i < idsReceived.size(); i++){
...
bool Game::movieAction(YAFScene* scene){
	...
	if((int)jogadasMovie.size() > movieAux){
