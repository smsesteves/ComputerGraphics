...
void YAFScene::switchTheme(){
	...
	**********/
	if(nextTheme == 1){ 
		for(unsigned int i = 0; i < this->appearancesComp.size(); i++){
		...
		for(unsigned int i = 0; i < this->appearancesComp.size(); i++){
			...
			if(appearancesComp[i]->getIdS() == "forest_celula"){
				for(unsigned int j = 0; j < this->tabCells.size(); j++){
			...
			if(appearancesComp[i]->getIdS() == "forest_casa_octi"){
				for(unsigned int j = 0; j < this->octiCells.size(); j++){
