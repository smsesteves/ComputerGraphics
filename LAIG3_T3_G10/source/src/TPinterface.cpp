...
//	cout << endl << "Turno: " << octi->turn << endl;
	...
	if(idpicado > 10 && idpicado < 18){
		...
		// JA TEM SELCCIONADO UMA DIRECAO - ADICIONAR PRONG
			...
			bool valid = false;
			for(unsigned int i = 0; i < octi->idsReceived.size(); i++){
	...
	if(idpicado > 110 && idpicado < 178){
		...
		// Jogador 1 vai mover peca
			...
			for(unsigned int i = 0; i < octi->getBoard()->Jogador1.size(); i++){
		...
		// Jogador 2 vai mover PEca
			...
			bool isInGame = false;
			// verifica se a peca que clicou antes esta em jogo
			for(unsigned int i = 0; i < octi->getBoard()->Jogador2.size(); i++){
	...
	if(octi->getEnded()){
		...
		// GANHOU JOG 1
		if(octi->turn == 2){
			for(unsigned int i = 0; i < ((XMLScene*) scene)->getScenePointer()->appearancesComp.size(); i++){
