...
void XMLScene::update(unsigned long	tempo)
{
	...
	for(unsigned int i = 0; i < scene->camerasComp.size(); i++){
	...
	if(octi->dificuldade < 0 && !doingcenas){
		...
		if(ultimoTempo == -1 && octi->gameStarted == true){
		...
		else if((unsigned int)numeroJogadas == octi->jogadas.size() && (int)((tempo-ultimoTempo)/1000) > tempoJogada && octi->gameStarted == true){
			...
			if((unsigned int)numeroJogadas < octi->jogadas.size()){
				ultimoTempo = tempo;
			}
			numeroJogadas = octi->jogadas.size();
